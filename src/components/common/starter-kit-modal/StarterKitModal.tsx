...
export const StarterKitModal: FunctionComponent<Props> = ({
  shopItem,
  onSubmit,
  isLoading,
}) => {
      ...
      <div className="z-10 mb-6 mt-auto flex flex-col items-center gap-3">
        <div
          className={classNames(
            "text-stroke-half mx-auto flex items-center gap-2 self-start rounded-full bg-gradient-to-tr from-[#FF8E01] to-[#DD342C] px-3 py-1 font-extrabold uppercase text-white text-shadow-sm",
          )}
        >
        ...
        <DrawerTitle>
          <div className="text-stroke-half px-5 text-center font-rubik text-[28px] font-black uppercase leading-none text-white text-shadow">
            {t(
              `${NS.PAGES.SHOP.STARTER_KIT_MODAL.ROOT}.${NS.PAGES.SHOP.STARTER_KIT_MODAL.TITLE}`,
            )}
          </div>
        </DrawerTitle>
        <DrawerDescription asChild>
          <p className="mb-6 text-center text-xs font-medium tracking-wide text-white">
            {t(
              `${NS.PAGES.SHOP.STARTER_KIT_MODAL.ROOT}.${NS.PAGES.SHOP.STARTER_KIT_MODAL.DESCRIPTION}`,
            )}
          </p>
        </DrawerDescription>
